...
public class settingController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("updateuser")){ 
			...
			if(res) {
				jsResponse("수정 성공", "setting.do?command=UserInfoForm", response);
				
			}else {
				jsResponse("수정 실패", "setting.do?command=ModifyForm", response);
			}
			...
			if(res) {
				jsResponse("탈퇴 성공", "loginController.do?command=logout", response);
			}else {
				jsResponse("탈퇴 실패", "setting.do?command=Deleteform&userno=<%=userno%>", response);
			}
